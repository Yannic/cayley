...
func (q *QuadStoreRegistration) New(addr string, options graph.Options) (graph.QuadStore, error) {
	return newQuadStore(addr, options)
}
...
func (q *QuadStoreRegistration) Init(addr string, opts graph.Options) error {
	return createNewMongoGraph(addr, opts)
}
